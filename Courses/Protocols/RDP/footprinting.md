...
```bash
nmap -sV -sC <ip_address> -p3389 --script rdp*
```
```bash
...
In addition, we can use --packet-trace parameter to track individuals packages.

```bash
nmap -sV -sC <ip_address> -p3389 --packet-trace --disable-arp-ping -n
```

| Parameter | Description |
|-----------|-------------|
| `--packet-trace` | Shows all packets sent and received for debugging |
